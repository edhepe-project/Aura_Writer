...
Incluye panel lateral con detalles de conexiones y creación/edición de rutas geográficas.
"""
...
class PlaceGraphDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self._info_desc = QLabel("Puedes hacer clic y arrastrar para mover libremente los lugares o usar 'Reorganizar'.")
        self._info_desc.setStyleSheet("font-size: 11px; color: #8e8e93;")
        self._info_desc.setWordWrap(True)
        pl.addWidget(self._info_desc)

        pl.addSpacing(10)
        pl.addWidget(QLabel("<b>Conexiones Geográficas:</b>"))
        self._connections_list = QListWidget()
        pl.addWidget(self._connections_list)

        # Formulario rápido para agregar conexión
        form_frame = QFrame()
        form_layout = QVBoxLayout(form_frame)
        form_layout.setContentsMargins(0, 4, 0, 0)
        form_layout.setSpacing(6)

        form_layout.addWidget(QLabel("<b>Añadir conexión desde aquí:</b>"))
        ...
        self._input_conn_label.setPlaceholderText("Nombre de la ruta/conexión (opcional)...")
    ...
    def _on_place_selected(self, place_id: str):
        ...
        # Mostrar lugar superior si es una estancia o sub-escenario
        if place.parent_place_id and place.parent_place_id in place_names:
            parent_item = QListWidgetItem(f"🏛️ Ubicado dentro de: {place_names[place.parent_place_id]}")
            parent_item.setForeground(QColor("#0a84ff"))
            self._connections_list.addItem(parent_item)

        # Mostrar estancias o sublugares contenidos
        sub_places = [p for p in places if p.parent_place_id == place_id]
        if sub_places:
            for sp in sub_places:
                sp_item = QListWidgetItem(f"🚪 Estancia / Interior: {sp.name} ({sp.category})")
        ...
        for lk in links:
            ...
            if other_id and other_id in place_names:
                lbl = f"➡️ {place_names[other_id]} ({lk.connection_type})"
                if lk.label:
                    lbl += f" — {lk.label}"
    ...
    def _add_connection(self):
        if not self._selected_place_id:
            QMessageBox.information(self, "Aviso", "Selecciona primero un lugar de origen en el mapa.")
            return

        target_id = self._combo_target_place.currentData()
        if not target_id or target_id == self._selected_place_id:
        ...
        existing = any(
            (lk.place_id_a == self._selected_place_id and lk.place_id_b == target_id) or
            (lk.place_id_b == self._selected_place_id and lk.place_id_a == target_id)
            for lk in meta.place_links
        )
        if existing:
            QMessageBox.information(self, "Aviso", "Ya existe una conexión entre estos dos lugares.")
            return

        new_link = PlaceLink(
            place_id_a=self._selected_place_id,
